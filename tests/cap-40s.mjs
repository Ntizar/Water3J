...
await new Promise(r => setTimeout(r, 40000)); // bien largo
const m = await page.evaluate(() => {
  ...
  return { calidad: W.estado.visual.calidad, visuales: W.matAgua.uniforms.uNumOlas.value, rango: +(mx-mn).toFixed(0) };
});
console.log('40s:', JSON.stringify(m));
await browser.close();

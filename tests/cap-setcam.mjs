...
await page.evaluate(() => { window.Water3J.aplicarEscena('huracan'); window.Water3J.setCamara({ angX: 0.55, dist: 70, auto: false }); });
await new Promise(r => setTimeout(r, 10000));
const cdp = await page.createCDPSession();
await cdp.send('Page.enable');
const shot = await cdp.send('Page.captureScreenshot', { format: 'png' });
writeFileSync('tests/v2-setcam.png', Buffer.from(shot.data, 'base64'));
const m = await page.evaluate(() => {
  const gl = document.querySelector('#escena canvas').getContext('webgl2');
  const w = gl.drawingBufferWidth, h = gl.drawingBufferHeight;
  const px = new Uint8Array(w * h * 4);
  gl.readPixels(0, 0, w, h, gl.RGBA, gl.UNSIGNED_BYTE, px);
  let mn=999, mx=0, n=0, s=0;
...
console.log('con setCamara:', JSON.stringify(m));
await browser.close();

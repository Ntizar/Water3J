...
const NOMBRE = 'Liencres (Playa del Madero)';
const C = { lat: 43.4637, lon: -3.9500 };   // Playa del Madero, Liencres (OSM); mar al NORTE
const radioKm = 0.7;
...
const spot = prog.find(s => s.spot.nombre.includes('Somo')) ?? prog[0];
let mejor = null;
...
writeFileSync('tests/ola-liencres.json', JSON.stringify({ C, nombre: 'Suances (Los Locos)', Hs0, T0, rej, rayos, roturas }, null, 1));
console.log(`3) ${rayos.length} rayos, ${roturas.length} roturas → tests/ola-liencres.json`);

...
await page.evaluate(() => {
  ...
  const g = new THREE.PlaneGeometry(1200, 1200, 96, 96); g.rotateX(-Math.PI/2);
  W.agua.geometry = g;
});
...
const m = await page.evaluate(() => {
  ...
  return { rango: +(mx-mn).toFixed(0), visuales: W.matAgua.uniforms.uNumOlas.value };
});
console.log('geo primero, huracán después:', JSON.stringify(m));
await browser.close();
